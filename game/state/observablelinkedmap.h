 ...
 */
template <class T> class ObservableLinkedMap final : public Observable
{
  public:
	ObservableLinkedMap() = default;
	virtual ~ObservableLinkedMap();

	ObservableLinkedMap operator!=(ObservableLinkedMap<T> other);
  ...
  protected:
	...
	std::unordered_map<UString, sp<T>> map;

...
struct Node
{
	...
	sp<Action> action_; // The action that got us here (for replay purposes)

	Node();
	Node(WorldState &state, int g, int h, int parent_id, const sp<Action> action);
...
{
	out << "Node { id:" << n.id_ << " parent:" << n.parent_id_ << " F:" << n.f() << " G:" << n.g_
	    << " H:" << n.h_;
	out << ", " << n.ws_ << "}";
	return out;
}

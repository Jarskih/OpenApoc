...
class Planner
{
  private:
	 ...
	 */
	std::vector<Node>::iterator memberOfOpen(const WorldState &ws);
	 ...
	 */
	int calculateHeuristic(WorldState &now, WorldState &goal) const;
  ...
  public:
	...
	*/
	...
	/**
	 ...
	 @return a vector of Actions in REVERSE ORDER - use a reverse_iterator on this to get
	 stepwise-order

...
int Planner::calculateHeuristic(WorldState &now, WorldState &goal) const
{
...
void Planner::addToOpenList(Node &&n)
{
	...
	auto it = std::lower_bound(begin(open_), end(open_), n);
...
{
	if (std::find_if(begin(closed_), end(closed_), [&](const Node &n) { return n.ws_ == ws; }) ==
	    end(closed_))
	{
		return false;
	}
...
std::vector<Node>::iterator Planner::memberOfOpen(const WorldState &ws)
{
                                 ...
                                 const std::vector<sp<Action>> &actions)
{
	...
	while (open_.empty() == false)
	{
		...
		printClosedList();
		std::cout << "\nCurrent is " << current << " : "
		          << (current.action_ == nullptr ? "" : current.action_->name()) << std::endl;
		...
		if (current.ws_.meetsGoal(*goal))
		{
			...
			do
			{
				...
				if (itr == end(open_))
				{
					itr = std::find_if(begin(closed_), end(closed_),
					                   [&](const Node &n) { return n.id_ == current.parent_id_; });
				}
				current = *itr;
		...
		for (auto &potential_action : actions)
		{
			if (potential_action->operableOn(current.ws_))
			{
				...
				{ // not a member of open list
					...
					Node found(outcome, current.g_ + potential_action->cost(),
					           calculateHeuristic(outcome, *goal), current.id_, potential_action);
				...
				{ // already a member of the open list
					...
					{
						std::cout << "My path to " << p_outcome_node->ws_ << " using "
						          << potential_action->name() << " (combined cost "
						          << current.g_ + potential_action->cost()
						          << ") is better than existing (cost " << p_outcome_node->g_
						          << "\n";
						...
						// sorting likely invalidates the p_outcome_node iterator, but we don't need
						// it anymore
